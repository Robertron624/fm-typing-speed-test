...
<div class="stats-and-options">
  <div class="current-stats">
    ...
    <div class="stat">
      ...
      <p class="value" class:imperfect={$isTestRunning && $accuracy < 100}>
        {$accuracy}%
      </p>
    </div>
    <div class="stat">
      <p class="label">
        {$testMode === "passage" ? "Time Elapsed:" : "Time Left:"}
      </p>
      <p
        class="value"
        class:time-warning={$isTestRunning &&
          $testMode === "timed" &&
          $timeLeft < 60 &&
          $timeLeft > 10}
        class:time-critical={$isTestRunning &&
          $testMode === "timed" &&
          $timeLeft <= 10}
      >
        {formatTime($timeLeft)}
  ...
  <div class="options">
    <div class="mobile">
      <div class="dropdown" use:clickOutside={() => (isDifficultyOpen = false)}>
        <button
          ...
          on:click={() => (isDifficultyOpen = !isDifficultyOpen)}
        >
          {$testDifficulty}
          <svg
      ...
      <div class="dropdown" use:clickOutside={() => (isModeOpen = false)}>
        <button
          class="dropdown-trigger"
          on:click={() => (isModeOpen = !isModeOpen)}
        >
    ...
    <div class="desktop">
      <div class="difficulty-options">
        ...
        <div class="buttons">
          <button
            class:active={$testDifficulty === "easy"}
            on:click={() => testDifficulty.set("easy")}>Easy</button
          >
          <button
            class:active={$testDifficulty === "medium"}
            on:click={() => testDifficulty.set("medium")}>Medium</button
          >
          <button
            class:active={$testDifficulty === "hard"}
            on:click={() => testDifficulty.set("hard")}>Hard</button
          >
        </div>
      </div>
      <div class="mode-options">
        ...
        <div class="buttons">
          <button
            class:active={$testMode === "timed"}
            on:click={() => testMode.set("timed")}>Timed (60s)</button
          >
          <button
            class:active={$testMode === "passage"}
            on:click={() => testMode.set("passage")}>Passage</button
          >
        </div>
      </div>
    </div>
